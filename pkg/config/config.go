...
func ParseConfig(path string) error {
	...
	if err != nil {
		return errors.Wrap(err, "to open config file: %v", path)
	}
...
func validateConfig(cfg *Config) error {
	...
	if err != nil || len(b) != 20 {
		return errors.Wrap(err, "expecting 40 hex character public address, got \"%s\"", cfg.PublicAddress)
	}
	if cfg.EnablePoolWorker {
		...
		if err != nil || len(b) != 32 {
			return errors.Wrap(err, "expecting 64 hex character private key, got \"%s\"", cfg.PrivateKey)
		}
		if len(cfg.ContractAddress) != 42 {
			return errors.Wrap(err, "expecting 40 hex character contract address, got \"%s\"", cfg.ContractAddress)
		}
