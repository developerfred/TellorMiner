...
func (a *ETHAddress) Set(v string) error {
	...
	if !valid {
		return errors.Wrapf(err, "%s is not a valid etherum address format", v)
	}
...
func (b *EthereumInt) Set(v string) error {
	...
	if !ok {
		return errors.Wrapf(err, "%s is not a valid integer", v)
	}
	if len(g.Bytes()) > 32 {
		return errors.Wrapf(err, "%s is larger than 256 bits", v)
	}
	b.Int = g
	return nil
}
